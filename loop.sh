...
# ═══════════════════════════════════════════════════════════════════════════════
# Workflow Mode Detection (Simple vs Advanced)
# ═══════════════════════════════════════════════════════════════════════════════
# Read mode from .ralph-mode file (defaults to 'simple' if not set)
WORKFLOW_MODE="simple"
if [ -f ".ralph-mode" ]; then
    WORKFLOW_MODE=$(cat ".ralph-mode" | tr -d '[:space:]')
    if [ "$WORKFLOW_MODE" != "simple" ] && [ "$WORKFLOW_MODE" != "advanced" ]; then
        WORKFLOW_MODE="simple"
    fi
fi
...
if [ "$MODE" = "plan-work" ]; then
    ...
    echo "Prompt:   $PROMPT_FILE"
    if [ "$WORKFLOW_MODE" = "simple" ]; then
        echo "Tasks:    prd.json + progress.txt"
    else
        echo "Tasks:    IMPLEMENTATION_PLAN.md"
    fi
    [ $MAX_ITERATIONS -gt 0 ] && echo "Max:      $MAX_ITERATIONS iterations"
...
# Count completed tasks based on workflow mode
count_completed_tasks() {
    if [ "$WORKFLOW_MODE" = "simple" ]; then
        # Simple mode: count passed stories in prd.json
        if [ ! -f "prd.json" ]; then
            echo 0
            return
        fi
...
# Count incomplete tasks based on workflow mode
count_incomplete_tasks() {
    if [ "$WORKFLOW_MODE" = "simple" ]; then
        # Simple mode: count non-passed stories in prd.json
        if [ ! -f "prd.json" ]; then
            echo 0
            return
        fi
